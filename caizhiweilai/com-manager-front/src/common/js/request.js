...
service.interceptors.response.use(
  res => {
    const backUrl = res.data.result
    endLoading() // 如果错误也结束动画
    // 是否退出标识
    // if (decodeURI(Cookies.get('czwlflag')) != '管理端' && Cookies.get('czwlflag')) {
    //   getCommonInfo().then(res => {
    //     // 返回用户中心地址 redirectUrl
      ...
      MessageBox.alert('登录信息超时,请重新登录!', '登录超时', {
        ...
        callback: action => {
          ...
          getCommonInfo().then(res => {
            ...
            location.href = backUrl + '?redirectUrl=' + Base64.encode(localStorage.getItem('centerUrl'))
          })
        }
      })
    }
...
async function request (opts) {
  ...
  if (['post', 'put', 'patch'].includes(opts.method.toLocaleLowerCase())) {
    const { url, method, params, headers, onUploadProgress } = opts
    try {
      if (opts.headers) {
