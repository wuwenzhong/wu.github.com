<template>
  <div class="yzf">
    <p class="time">
      <span v-if="currentType == 6">{{timeTitle}}：{{showTaskTime}}</span>
    ...
    <div
      ...
      v-show="currentType == 6 && (trainState == '1' || trainState == '0')"
    >
      <div class="RobotImg" :class="[beginMove?'move':'',beginTransform?'isTransform':'']">
...
export default {
  ...
  data() {
    return {
      ...
      rpaCode: this.$route.query.rpaCode,
      trainState: this.$route.query.trainState, // 实训状态，0,1代表着没有完成实训2的时候完成了任务
      ...
      ckfileArr: []
      /* 使用打开新窗口的方法时使用 */
      // url: localStorage.getItem('iframeUrl')
    }
  },
  computed: {
  ...
  methods: {
    ...
    async achieve() {
      ...
      if (this.$route.query.caseId) {
        that.caselist.teamId = 0
        that.caselist.userId = localStorage.getItem('userId')
        that.caselist.classId = localStorage.getItem('classId')
        that.caselist.caseId = this.$route.query.caseId
        // 班级任务id
        that.caselist.classTaskId = this.$route.query.id
...
<style lang="scss" scoped>
  ...
  .yzf {
    ...
    /*background-image: url("../../common/images/computer-screen1.png");*/
    ...
    .Robot {
        ...
        &.isTransform {
          ...
          .step {
            ...
            .vip-control-header:after {
              border-color: transparent transparent transparent rgba(102, 102, 102, 0.3);
              left: initial;
              right: -0.194rem;
            }
          }
      ...
      .step {
        ...
        .vip-control-header:after {
          ...
          border-color: transparent rgba(102, 102, 102, 0.3) transparent transparent;
          z-index: 111;
        }
