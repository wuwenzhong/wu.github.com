...
import { ssoGetToken, userLogin, application } from 'api/auth'
...
router.beforeEach(async (to, from, next) => {
  ...
  if (startStatus == '3' && (to.path === '/jxgl/index' || to.path === '/sxgk/index' || to.path === '/teachplan/index')) {
    next({ path: from.path })
    store.commit('setDialogInit', true)
  } else {
    next()
  }

  if (startStatus == '3' && to.path === '/home/index') {
    store.commit('setDialogInit', true)
  } else {
    next()
  }
