<template>
  <div class="paper-list-wrapper">
      ...
      <li class="test-list" v-for="(item, index) in paperData" :key="item.id">
        <div class="test-title">
          ...
          <div class="test-statistics">
            <i>{{ item.questionExamDTO.accuracy + "%" }}</i
            >（{{ item.questionExamDTO.successNum }} /
            {{ item.questionExamDTO.count }}）
          </div>
        </div>
        <dl v-if="item.questionBankDTO.questionType == 2" class="judge-test">
          <dd
            v-for="ele in item.questionBankDTO.questionOptionDTOList"
            :key="ele.id"
          >
        ...
        <dl v-else>
          <dd
            ...
            :key="ele.id"
          >
            <span>{{ ele.optionCode }}：</span>
            <p>{{ ele.optionContent }}</p>
          </dd>
        </dl>
        <div
          class="test-analysis"
          v-if="item.questionBankDTO.questionType == 2"
        >
          <div>
            <span>正确答案：<i></i></span
            ><em
              v-html="item.questionBankDTO.answer == 'A' ? '正确' : '错误'"
            ></em>
          </div>
          <p>
            <span>答案解析：<i></i></span
            >{{ item.questionBankDTO.answerAnalysis }}
          </p>
        </div>
        <div class="test-analysis" v-else>
          <div>
            <span>正确答案：<i></i></span
            ><em
              v-for="(k, indexs) in item.questionBankDTO.answer"
              :key="indexs"
    ...
    <Pagination
      ...
      :total="paperTotal"
      :page.sync="paperPage"
      :limit.sync="paperSize"
    ></Pagination>
  </div>
</template>

<script>
import { lookExamDetail } from '@/api/teamAdmin'
      ...
      .test-analysis {
        ...
        span {
          ...
          i {
            width: 100%;
            height: 10px;
            background-color: rgba(65, 214, 107, 0.2);
            position: absolute;
            bottom: 0;
            left: 0;
