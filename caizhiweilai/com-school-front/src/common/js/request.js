...
service.interceptors.response.use(
  res => {
    ...
    // 是否退出标识
    if (decodeURI(Cookies.get('czwlflag')) != '教务端' && Cookies.get('czwlflag')) {
      getCommonInfo().then(res => {
        // 保存返回用户中心地址 redirectUrl
        localStorage.setItem('centerUrl', res.result.redirectUrl)
        if (res.code == 0 || res.code == '0') {
          // location.href = res.result.redirectUrl + localStorage.getItem('centerUrl') + '/usercenter/applicationCenter'
          location.href = localStorage.getItem('centerUrl')
        }
      })
    }
...
async function request (opts) {
  ...
  if (['post', 'put', 'patch'].includes(opts.method.toLocaleLowerCase())) {
    const { url, method, params, headers } = opts
    try {
      if (opts.headers) {
