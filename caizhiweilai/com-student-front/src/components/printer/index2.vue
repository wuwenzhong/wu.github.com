...
<div class="inner draw" @mousemove="beginPath($event)">
  <div class="wrap">
    <div id="control" class="tool-control">
      <!--画笔颜色-->
      <div class="tool-list">
        <span class="list-name">画笔颜色</span>
        <colorPicker v-model="color" class="choose-color" v-on:change="setColor" />
      </div>
      <!--画笔-->
      <div class="tool-list">
        <span class="list-name">画笔线宽</span>
        <input type="number" min="1" step="1" class="line-width" v-model="config.lineWidth" placeholder="请输入线宽" @blur="testInput">
      </div>
      <!--操作-->
      <div class="tool-list">
        <span class="list-name">操作</span>
        <span v-for="(control,index) in controls"
              :title="control.title"
              class="iconfont"
              :class="control.className"
              @click="controlCanvas(control.action)"
              v-html="control.text" :key="index">
        </span>
      </div>
      <!--形状-->
      <div id="canvas-shape">
...
export default {
  ...
  data() {
    return {
      ...
      lineBiao: [], // 直线坐标
      criculBiao: [], // 圆坐标
      printStatus: false, // 记录画笔状态，禁止mouseMove之后就开始画
      fileList: []
    }
  },
  created() {
      ...
      this.cos(files).then(res => {
        ...
        wjUp({
          ...
          if (res.code == 0) {
            ...
            localStorage.setItem('wjIdArr', this.fileList)
            // 关闭会话框
            this.$emit('cancel', this.imgUrl)
          }
        })
      })
      // 生产的文件路径
